...
keyword [] input = Left "Empty keyword"
keyword kw input =
...
and6 p1 p2 p3 p4 p5 p6 input =
  case and5 p1 p2 p3 p4 p5 input of
    Left e -> Left e
      ...
      case cmd of
        Lib1.Report rc ->
          case skipWhitespace rest1 of
            Right (_, rest2) ->
              case parseReportList rest2 of
                Left _ -> Right ([rc], rest2)
                Right (more, rest3) -> Right (rc : more, rest3)
            Left e -> Left e
...
instance ToCliCommand Lib1.Command where
  ...
  toCliCommand (Lib1.Schedule (Lib1.ScheduleAt deviceName action time)) = "schedule " ++ deviceName  ++
    case action of { Lib1.TurnOnDevice -> " turn on " ;
        Lib1.TurnOffDevice -> " turn off " ;
        Lib1.SetBrightnessLevel -> " set brightness " ;
         Lib1.SetTemperatureLevel -> " set temperature " } ++ show time
...
instance Eq Lib1.Command where
  ...
  Lib1.Set (Lib1.SetBrightness d1 b1) == Lib1.Set (Lib1.SetBrightness d2 b2) = d1 == d2 && b1 == b2
  Lib1.Set (Lib1.SetTemperature d1 t1) == Lib1.Set (Lib1.SetTemperature d2 t2) = d1 == d2 && t1 == t2
  Lib1.Set (Lib1.SetState d1 Lib1.On)  == Lib1.Set (Lib1.SetState d2 Lib1.On) = d1 == d2
  Lib1.Set (Lib1.SetState d1 Lib1.Off) == Lib1.Set (Lib1.SetState d2 Lib1.Off) = d1 == d2
  ...
  Lib1.Schedule (Lib1.ScheduleAt d1 Lib1.TurnOnDevice t1) == Lib1.Schedule (Lib1.ScheduleAt d2 Lib1.TurnOnDevice t2) = d1 == d2 && t1 == t2
  Lib1.Schedule (Lib1.ScheduleAt d1 Lib1.TurnOffDevice t1) == Lib1.Schedule (Lib1.ScheduleAt d2 Lib1.TurnOffDevice t2) = d1 == d2 && t1 == t2
  Lib1.Schedule (Lib1.ScheduleAt d1 Lib1.SetBrightnessLevel t1) == Lib1.Schedule (Lib1.ScheduleAt d2 Lib1.SetBrightnessLevel t2) = d1 == d2 && t1 == t2
  Lib1.Schedule (Lib1.ScheduleAt d1 Lib1.SetTemperatureLevel t1) == Lib1.Schedule (Lib1.ScheduleAt d2 Lib1.SetTemperatureLevel t2) = d1 == d2 && t1 == t2
  Lib1.Report (Lib1.ReportHouse h1 rs1) == Lib1.Report (Lib1.ReportHouse h2 rs2) = h1 == h2 && and (zipWith (==) (map Lib1.Report rs1) (map Lib1.Report rs2))
  Lib1.Report (Lib1.ReportRoom r1 rs1) == Lib1.Report (Lib1.ReportRoom r2 rs2) = r1 == r2 && and (zipWith (==) (map Lib1.Report rs1) (map Lib1.Report rs2))
  Lib1.Report (Lib1.ReportDevice d1) == Lib1.Report (Lib1.ReportDevice d2) = d1 == d2
